...
(define-type BFAE
  ...
  [app (f BFAE?) (a BFAE?)]
  [newbox (v BFAE?)]
  [setbox (b BFAE?) (v BFAE?)]
  [openbox (b BFAE?)]
  [seqn (s1 BFAE?) (s2 BFAE?)])
...
(define (store-lookup addr st)
  (type-case Store st
    [mtSto () (error 'box-lookup "no store value at ~a" box)]
...
; BFAE DefrdSub Store -> Value*Store(v*s)
(define (interp bfae ds st)
  (type-case BFAE bfae
    ...
    [id (x) (v*s (lookup x ds) st)]
    
    [fun (p b) (v*s (closureV p b ds) st)]
    
    [app (f a) (interp2 f a ds st
                        (lambda (v1 v2 s2)
                          (interp (closureV-body v1) (aSub (closureV-param v1) v2 (closureV-ds v1)) s2)))]
             
    [newbox (v) (local [(define new-addr (malloc st))]
                  (type-case Value*Store (interp v ds st)
                    [v*s (v s) (v*s (boxV new-addr) (aSto new-addr v s))]))]
